...
// Function to check if rover reached the target
int reachedTarget(struct Rover rover) {
    if (rover.x == TARGET_X && rover.y == TARGET_Y) {
        return 1;  // Yes, rover reached target
    } else {
        return 0;  // No, target not reached
    }
}
...
int distanceToTarget(struct Rover rover) {
    ...
    // Return the sum of absolute distances (Manhattan distance)
    if (dx < 0) dx = -dx;  // Make dx positive
    if (dy < 0) dy = -dy;  // Make dy positive
...
// Function to scan the area around the rover
void scanArea(struct Rover *rover) {
    ...
    temperature = 50 + rand() % 60;  // Random between 50-109 Celsius
    terrain_type = rand() % 3;        // Random between 0, 1, 2
    rocks_detected = rand() % 5;      // Random between 0-4 rocks
...
void moveNorth(struct Rover *rover, char map[GRID_SIZE][GRID_SIZE]) {
    ...
    // Check if rover reached target after moving
    if (reachedTarget(*rover)) {
        printf("\n*** TARGET REACHED! ***\n");
